...
<<<<<<< HEAD
      res['column'] = tempColumns.length && this.siteLinkItems[0] && this.siteLinkItems[0].length ? Math.max(...tempColumns
        .map(el => this.siteLinkItems[el])
        .reduce((a, b) => (a || []).concat(b || []))
        .map(el => el.info.column)) : 1;
      res['row'] = (tempColumns.length && this.siteLinkItems[0] && this.siteLinkItems[0].length ? Math.max(...tempColumns
=======
