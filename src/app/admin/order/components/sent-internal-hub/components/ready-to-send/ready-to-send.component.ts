...
export class ReadyToSendComponent implements OnInit {
  ...
  displayedColumns = ['position', 'customer', 'order_time', 'total_order_lines', 'process' ];
  expandedElement: any;
  ...
  invoicing(row) {
    this.expandedElement = row._id; // for show highlight style
    this.isBarcodeChecker = true;
  }
